...
class TvShowDetailViewModel @ViewModelInject constructor(
    @Assisted private val savedStateHandle: SavedStateHandle,
    repository: MovieDbRepository,
    app: Application
) : AndroidViewModel(app) {

    fun saveMovieId(tvShowId: Long) {
        savedStateHandle.set(TV_SHOW_KEY, tvShowId)
    }

    val tvShowDetail: LiveData<TvShowDetail?> = savedStateHandle.getLiveData<Long>(TV_SHOW_KEY)
        .switchMap {
            liveData {
    ...
    companion object {
        private const val TV_SHOW_KEY = "tvShowId"
    }
